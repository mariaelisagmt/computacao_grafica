...
//Adiciona as coordenadas dos pontos de uma linha no plano representada por dois pontos
void adicionarPontos(GLfloat x1, GLfloat y1){
	linhas.push_back(x1);
	linhas.push_back(y1);
}

//Apagar as coordenadas dos pontos de uma linha no plano representada por dois pontos
void apagarPontos(int x, int y){
	int x1, y1, x2, y2, res = 0; // linha
	//Verificar a Intersecao do ponto com as retas
	for (it = linhas.begin(); it != linhas.end();){
    //reta 1
...
{
	/*adicionarLinha(0.0f, -0.5f, 0.43f, -0.25f);
	adicionarLinha(0.43f, -0.25f, 0.43f, 0.25f);
	adicionarLinha(0.43f, 0.25f, 0.0f, 0.5f);
	adicionarLinha(0.0f, 0.5f, -0.43f, 0.25f);
	adicionarLinha(-0.43f, 0.25f, -0.43f, -0.25f);
	adicionarLinha(-0.43f, -0.25f, 0.0f, -0.5f);*/
	try {		
	...
	try {
		proxy->useProgram();
		proxy->setAttribute("posicao", &linhas[0], linhas.size()
		 * sizeof(GLfloat), coordenadasPorVertices);
		proxy->setUniform4f("cor",0.0f, 2.0f, 0.8f, 1.0f);
...
void tratarEventoDoMouse(int button, int state, int x, int y){
	...
	*/
	if (state == GLUT_DOWN && GLUT_LEFT_BUTTON == button){
	  /*Transformação das coordenadas do objeto para as da camera*/
	  if(linhas.empty())
	    adicionarPontos(nx,ny);
	  else
	  	adicionarLinha(nx,ny,nx,ny);
	  cout<<"Point: ("<<nx<<", "<<ny<<")"<<endl;
 	}
 	else{
